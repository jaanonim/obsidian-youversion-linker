...
import { bookRegex, linkRegex, chapterSeparatorRegex, rangeSeparatorRegex } from './Regex';
...
import { makeRegexTypeList, makeVerseByType, VerseType } from './verses/VerseType';
...
export class EditorSuggester extends EditorSuggest<VerseLink> {
  ...
  getSuggestions(context: EditorSuggestContext): VerseLink[] | Promise<VerseLink[]> {
    ...
    return getSuggestionsFromQuery(query.substring(1), verseType, this.settings);
  }
...
export function processVerses(verses_str: Array<string>): Array<VerseElement> {
  return verses_str
    .map((verse) => {
      ...
      return start === undefined ? undefined : new VerseElement(start, end);
    })
...
export function getSuggestionsFromQuery(
  ...
  const [chapter_str, ...verses_str] = numbersPartsOfQueryString.split(chapterSeparatorRegex);
